...
class API:
    ...
    def __init__(self, username, password, path):
        ...
        # list of dictionaries that contains all the information about the plans
        self.plans = requests.get(API.url_plan, 
                    headers=self.headers).json()['results'] 
        # list of dictionaries that contains all the information the meals
        self.meals = requests.get(API.url_meal, 
                    headers=self.headers).json()['results'] 
        # list of dictionaries that contains all the information the items
        self.items = requests.get(API.url_items, 
                    headers=self.headers).json()['results'] 
        # list on dictionaries that contains all the information the workouts
        self.workouts = requests.get(API.url_workout, 
                    headers=self.headers).json()['results'] 
        # list of dictionaries that contains all the information about the days
        self.days = requests.get(API.url_day, 
                    headers=self.headers).json()['results'] 
        # list of dictionaries that contains all the information about the exercises
        self.sets = requests.get(API.url_sets, 
                    headers=self.headers).json()['results'] 
        # list of dictionaries that contains all the information about a schedule
        self.schedules = requests.get(API.url_schl, 
                    headers=self.headers).json()['results'] 
        # list of dictionaries that contains all the information about weights
        self.weights = requests.get(API.url_weight,
                    headers=self.headers).json()['results']
        # list on dictionaries that contains all the information about a exercice
        self.exercises = requests.get(API.url_exercise, 
                    headers=self.headers).json()['results'] 
        # list of dictionaries that contains all the information about links
        self.links = requests.get(API.url_link, 
                    headers=self.headers).json()['results'] 
        # list of dictionaries that cointains all the comments about exercises
        self.comments = []
        # dictionary that contains all the entries added in this session  
        self.curent_sesion = {}
    ...
    def add_post(self, payload, url, info, **kwargs):
        """
        ...
        self.headers['Referer'] = kwargs.get('extra')
        check = requests.post(url, data=payload, headers=self.headers, timeout=2)
        app = check.json()
        info.append(app)
        if check.ok == True:
            # Checking post request
            if requests.get(url, header=self.headers).json()['results'] != info:
                print("Couldn't add " + str(payload) + " properly")
                info.pop()
                return False
            else:
                if url not in self.curent_sesion:
    ...
    def add_weight(self):
        ...
        if entries:
            for payload in entries:
                ...
                payload['csrfmiddlewaretoken'] = self.extract_csrf\
                                            ('https://wger.de/en/weight/add/')
                # Test the entry with it's json schema
                check.check_entry(path='schemas/weight.json', test=payload)
                # Post request
                self.add_post(payload, API.url_weight, self.weights, extra=API.url_weight)
    ...
    def add_plan(self):
        ...
        if plans :
            ...
            for payload in plans:
                ...
                # Post request
                output.append(self.add_post(ready, API.url_plan, self.plans))
                # Check for meals
                if 'meal' in payload.keys() and payload['meal'] != [{}]:
                   output.append(self.add_meal(self.plans[-1].get('id')))                     
        return all(output)
    ...
    def add_meal(self, p_id):
        ...
        for entries in plans:
            for payload in entries.get('meal',{}):
                ...
                # Post request
                output.append(self.add_post(ready, API.url_meal, self.meals))
                # Check for items
                if 'item' in payload.keys() and payload['item'] != [{}]:
                    output.append(self.add_item(self.meals[-1].get('id')))
        return all(output)
    ...
    def add_workout(self):
        ...
        if workouts :
            ...
            for payload in workouts:
                ...
                # Post request
                output.append(self.add_post(ready, API.url_workout, self.workouts))
                # Check for days
                if 'day' in payload.keys() and payload['day'] != [{}]:
                    output.append(self.add_day(self.workouts[-1].get('id')))
        return all(output)
        ...
        if workouts:
            for entries in workouts:
                ...
                for payload in entries.get('day',{}):
                    ...
                    # Post request
                    output.append(self.add_post(ready, API.url_day, self.days))
                    # Check for sets
                    if 'sets' in payload.keys() and payload['sets'] != [{}]:
                        output.append(self.add_sets(self.days[-1].get('id')))
        return all(output)
    ...
    def add_schedule(self):
        ...
        if entries:
            ...
            for payload in entries:
                ...
                # Post request
                output.append(self.add_post(ready, API.url_schl, self.schedules)) 
                if 'link' in payload.keys() and payload['link'] != [{}]:
                    output.append(self.link(self.schedules[-1].get('id')))
        return all(output)
    ...
    def get_image(self, index):
        ...
        image = requests.get(API.url_image, headers=self.headers).json()['results']
