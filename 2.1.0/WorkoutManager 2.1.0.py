    ...
    def add_plan(self):
        ...
        if plans :
            ...
            for payload in plans:
                ...
                # Check for meals
                if payload['meal']:
                    for meal in payload['meal']:
                        # Parse the payload
                        ready2 = self.construct_payload(parse = copy.deepcopy(meal), dele='item')
                        # Check the entry vs a json schema
                        check.check_entry(path='schemas/meal.json', test=ready2)
                        # Post request
                        self.add_post(ready2, API.url_meal, self.meals)
                ...
                for payload in entries.get('meal',{}):
                    ...
                    if payload['item']:
                        for item in payload['item']:
                            # Check the entry vs a json schema
                            check.check_entry(path='schemas/item.json', test=item)
                            # Post request
                            self.add_post(item, API.url_items, self.items)
    ...
    def add_workout(self):
        ...
        if workouts :
            ...
            for payload in workouts:
                ...
                # Post request
                self.add_post(ready, API.url_workout, self.workouts)
                # Check for days
                if payload['day']:
                    for day in payload['day']:
                        # Parse the payload
                        ready = self.construct_payload(parse = copy.deepcopy(day), dele='sets')
                        # Check the entry vs a json schema
            ...
            for entries in workouts:
                if entries:
                    ...
                    for payload in entries.get('day',{}):
                        ...
                        # Check for sets
                        if payload['sets']:
                            for sets in payload['sets']:
                                # Check the entry vs a json schema
                                check.check_entry(path='schemas/set.json', test=sets)
                                # Post request
                                self.add_post(sets, API.url_sets, self.sets)
